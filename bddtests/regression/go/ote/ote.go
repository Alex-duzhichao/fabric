...
import (
	...
	genesisconfig "github.com/Alex-duzhichao/fabric/common/configtx/tool/localconfig" // config for genesis.yaml
	genesisconfigProvisional "github.com/Alex-duzhichao/fabric/common/configtx/tool/provisional"
	"github.com/Alex-duzhichao/fabric/orderer/localconfig" // config, for the orderer.yaml
...
func ote(testname string, txs int64, chans int, orderers int, ordType string, kbs int, masterSpy bool, pPerCh int) (passed bool, resultSummary string) {
	...
	if numChannels == 1 {
		...
		for c := 0; c < numChannels; c++ {
			...
			cmd := fmt.Sprintf("cd $GOPATH/src/github.com/Alex-duzhichao/fabric && peer channel create -c %s", channelIDs[c])
			_ = executeCmd(cmd)
